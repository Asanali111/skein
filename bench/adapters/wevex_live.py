"""Wevex adapter against a live, already-running daemon.

Read-only — never mutates the user's real data. Reads the bearer token and
port from ``~/.config/wevex/config.json``.
"""
...
class WevexLiveAdapter(ReadOnlyAdapter):
    """Hits the live Wevex daemon at ``http://host:port`` over real HTTP."""
    ...
    def _load_config() -> dict:
        path = Path.home() / ".config" / "wevex" / "config.json"
        if not path.exists():
            return {}
        try:
