"""Ephemeral Wevex adapter — fresh in-process daemon backed by a temp SQLite DB.
...
class WevexEphemeralAdapter(MutableAdapter):
    """In-process Wevex backed by ``tempfile.mkdtemp`` SQLite + hash embeddings."""

    name = "wevex-ephemeral"
    ...
    def _open(self) -> None:
        ...
        from wevex.models import IdentityCreate

        self._tmpdir = Path(tempfile.mkdtemp(prefix="bench_wevex_"))
        db_path = self._tmpdir / "wevex.db"
    ...
    def recall(
        ...
        *,
        ...
        from wevex.retrieval import recall as do_recall
    ...
    def search_code(
        ...
        *,
        ...
        from wevex.models import ChunkSearchRequest
    ...
    def would_capture_commit(self, subject: str, body: str = "") -> bool:
        from wevex.git_watcher import GitCommit, is_noise_commit
        c = GitCommit(
    ...
    def claim_lease(self, glob: str, *, scope: str, ttl_seconds: int = 60) -> Optional[str]:
        from wevex.models import LeaseCreate
