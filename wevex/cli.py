"""Wevex CLI — all user-facing commands.
...
Visible commands:
  ...
  connect    Wire installed LLM tools through Wevex (--remove to disconnect).
"""
...
# and aren't used by hot commands like `wevex --help`, `--version`, or status.
...
def _require_running(client) -> bool:
    ...
    try:
        ...
        err_console.print(
            "[bold red]✗[/bold red] Wevex daemon is not running. "
            "Start it with [bold]wevex serve[/bold]."
        )
        sys.exit(1)
...
def _resolve_self_bin() -> str:
    """Best-effort path to the `wevex` executable for embedding in hooks/launchd.

    Prefer ``/usr/local/bin/wevex`` (the symlink the installer creates) so the
    written hooks survive the venv being moved or rebuilt.  Fall back to the
    venv binary, then to plain ``"wevex"``.
    """
    import shutil
    candidates = [
        "/usr/local/bin/wevex",
        str(Path.home() / ".local" / "bin" / "wevex"),
    ]
    for c in candidates:
        if Path(c).is_file():
            return c
    found = shutil.which("wevex")
    return found or "wevex"


def _resolve_scope(cli_scope: Optional[str]) -> str:
    """Resolve the active scope handle for any CLI command.

    Honors (in order): --scope flag, WEVEX_SCOPE env, .wevex/scope pin, config default.
    ...
    # Soft hint: when the user didn't pass --scope but a .wevex/scope pin took effect,
    # show a dim line so it's not surprising. Skip for noisy commands (recall/search).
    if source == "pin" and cli_scope is None and not os.environ.get("WEVEX_QUIET_PIN"):
        err_console.print(
            f"[dim]using scope [cyan]{scope}[/cyan] from .wevex/scope[/dim]"
        )
    return scope
...
@click.version_option(package_name="wevex", prog_name="wevex")
@click.pass_context
def main(ctx: click.Context) -> None:
    ...
    \b
    Quick start:
        wevex up              # start daemon, connect clients, watch this repo
        wevex status          # see what's wired up
        wevex doctor          # deep diagnostic
        wevex briefing        # what's the state of this project?
        wevex down            # stop everything
...
def up(
    ...
    What it does:
      ...
      4. Install autonomous hooks (.claude/settings.json + .cursor/rules + .wevex/scope)
      5. Sync MCP configs to all installed LLM clients
      6. Ingest the codebase for RAG
    """
    from .agents_md import render_agents_md
    from .auth import generate_token
    from .config import WevexConfig, _default_config_path, load_config
    ...
    if refusal and not no_ingest:
        ...
        _guard_ui.hint(
            "Run [bold]wevex up[/bold] from a real project directory "
    ...
    if (
        ...
        and os.environ.get("WEVEX_ALLOW_NO_GIT") != "1"
    ):
        err_console.print(
        ...
        _guard_ui.hint(
            "Run [bold]wevex up[/bold] from inside a git repo, OR "
            "use [bold]wevex up --no-ingest[/bold] to skip indexing, OR "
            "set [bold]WEVEX_ALLOW_NO_GIT=1[/bold] if you really mean it."
        )
        sys.exit(1)
    ...
    # auto-relocate to ~/.wevex/venv before starting the service.
    wevex_bin_for_daemon = _resolve_self_bin()
    if not no_persist:
        from .daemon import is_tcc_protected_path, relocate_venv_to_wevex_home
        if is_tcc_protected_path(Path(wevex_bin_for_daemon)):
            console.print(
                "[yellow]⚠[/yellow] Your wevex install is inside a macOS-protected "
                f"folder ([dim]{Path(wevex_bin_for_daemon).parent.parent}[/dim]).\n"
                "    Relocating to [bold]~/.wevex/venv[/bold] so launchd can run it…"
            )
            with console.status("[dim]Building TCC-safe venv at ~/.wevex/venv…[/dim]",
                                spinner="dots"):
                try:
                    new_bin = relocate_venv_to_wevex_home()
                except Exception as e:
                    err_console.print(
                        f"[red]✗[/red] Relocation failed: {e}\n"
                        f"    Falling back to non-persistent (nohup) daemon. "
                        f"Run wevex up again later or move the venv manually."
                    )
                    no_persist = True
                else:
                    wevex_bin_for_daemon = str(new_bin)
    ...
    with console.status(f"[dim]Ensuring daemon is running ({method_label})…[/dim]",
        ...
        try:
            status = ensure_running(
                ...
                wevex_bin=wevex_bin_for_daemon,
            )
    ...
    if status.healthy:
        ...
        err_console.print(
            ...
            f"(method={status.method}). See ~/.config/wevex/logs/daemon.err"
        )
        sys.exit(1)
    ...
    try:
        ...
        # ---- 5. hooks ----
        if not no_hooks:
            wevex_bin = _resolve_self_bin()
            report = install_hooks(
                repo_path=repo_path, scope_handle=scope_handle,
                wevex_bin=wevex_bin, user_global=user_global,
            )
        ...
        # `agents_md_sync_interval` (60 s default). `wevex connect` is the
        ...
        if not no_sync and not was_already_healthy:
            ...
            if not connected:
                console.print(
                    ...
                    "Run [bold]wevex connect[/bold] to pick which tools should "
                    "share context, then re-run [bold]wevex up[/bold]."
                )
            else:
        ...
        # pollute the user's real ~/.config/wevex/projects.json.
        if not no_persist:
            ...
            if watcher_manager.is_running(entry):
                ...
                try:
                    pid = watcher_manager.spawn(
                        entry, wevex_bin=_resolve_self_bin(),
                    )
                    if pid:
                        console.print(
                    ...
                    err_console.print(
                        ...
                        "    Manual `wevex ingest` will still work."
                    )
        ...
        if run_ingest:
            ...
            if stats.embedding_degraded:
                ...
                ui.hint(
                    ...
                    "[bold]wevex ingest .[/bold] later (or set "
    ...
    # `wevex/server.py::_passive_scan_loop` so `wevex up` returns as soon
    ...
    # ---- 8. friendly summary ----
    ui.header("Wevex is ready", state="ok")
    home = str(Path.home())
    ui.fields([
    ...
    ui.blank()
    ui.hint(
        "Try [bold]wevex recall \"<query>\"[/bold] · "
        "[bold]wevex search \"<query>\"[/bold] · "
        "[bold]wevex down[/bold] to stop"
    )
...
def restart() -> None:
    ...
    if status.healthy:
        ...
        err_console.print(
            ...
            f"See ~/.config/wevex/logs/daemon.err"
        )
        sys.exit(1)
...
def update(check_only: bool) -> None:
    """Upgrade Wevex to the latest version and restart the daemon.

    Detects how Wevex was installed (pipx / uv tool / pip) and runs the
    ...
    import shutil
    wevex_bin = shutil.which("wevex") or sys.executable
    wevex_path = Path(wevex_bin).resolve()

    # Check for editable / dev install (source tree).
    is_editable = False
    try:
        import importlib.metadata as _meta
        dist = _meta.distribution("wevex")
    ...
    if is_editable:
        ui.blank()
        console.print(
            "  [yellow]⚠[/yellow] Wevex is installed in editable/dev mode.\n"
            "  Pull the latest changes and restart the daemon:\n"
            "\n"
            "    [bold]git pull && wevex restart[/bold]\n"
            "\n"
            "  If you installed via the TCC workaround (macOS) also run:\n"
            "    [bold]wevex down && rm -rf ~/.wevex/source && wevex up[/bold]"
        )
        ui.blank()
        return

    # Detect pipx: binary lives inside a pipx venv
    #   ~/.local/pipx/venvs/wevex/bin/wevex  or  ~/Library/Application Support/pipx/…
    is_pipx = "pipx" in str(wevex_path) or "pipx" in os.environ.get("PIPX_HOME", "")
    if not is_pipx:
        # Secondary check: pipx creates a link under ~/.local/bin
        try:
            resolved = wevex_path.resolve()
            is_pipx = "pipx" in str(resolved)
        except Exception:
            pass

    # Detect uv tool: binary lives inside ~/.local/share/uv/tools/wevex/
    is_uv_tool = "uv" in str(wevex_path) and "tools" in str(wevex_path)
    if not is_uv_tool:
        try:
            is_uv_tool = "uv" in str(wevex_path.resolve()) and "tools" in str(wevex_path.resolve())
        except Exception:
            pass

    if is_pipx:
        upgrade_cmd = ["pipx", "upgrade", "wevex"]
    elif is_uv_tool:
        upgrade_cmd = ["uv", "tool", "upgrade", "wevex"]
    else:
    ...
    # Invalidate the update-check cache so next `wevex status` shows up to date.
    try:
    ...
    if restart_status.healthy:
        ...
        ui.step(
            ...
            detail="run `wevex restart` manually",
            state="warn",
        )
    ui.blank()
...
def _disconnect_impl(client_id: Optional[str], all_connected: bool) -> None:
    """Remove Wevex from one or all connected LLM tools.

    \b
    Forms:
      wevex disconnect cursor    surgically remove wevex from cursor configs
      wevex disconnect --all     disconnect everything
    """
    from . import clients as clients_mod
    from . import connections as conns
...
def _since_impl(
    ...
    Examples:
        wevex since 1h
        wevex since 2d --exclude-tool claude_code
        wevex since 2026-05-12 --type decision --limit 20
    """
...
def _doctor_clean() -> None:
    """ADR-002: replace `wevex gc` invocation. Delegates to the existing
...
def _doctor_reingest() -> None:
    """ADR-002: replace `wevex ingest .` invocation. Delegates to the
...
def _doctor_reindex_embeddings() -> None:
    ...
    try:
        ...
        ui.hint("Restart the daemon (wevex restart) so cached query "
                "embeddings flush.")
    finally:
        try:
            storage.close()
...
def _gc_impl(yes: bool, dry_run: bool) -> None:
    ...
    try:
        ...
        # ---- Report ----
        ui.section("Wevex garbage collection")
        ui.blank()
        ui.fields([
        ...
        ui.step(f"Deleted {deleted_scopes} scopes, {deleted_frags} fragments",
                state="ok")
        ui.hint(
            "Run [bold]wevex chunks delete-scope[/bold] for any scope "
...
def _ingest_impl(
    ...
    \b
    Examples:
        wevex ingest ~/Documents/myapp
        wevex ingest ./src --include .py,.md --scope project:myapp
        wevex ingest . --reset --prune        # full re-index
        wevex ingest . --dry-run              # see what would be done
...
def watch(path: str, scope: str, source_root: Optional[str], polling: bool) -> None:
    ...
    \b
    Internal use: ``wevex up`` spawns this as a detached subprocess.
...
def init(
    ...
    \b
    After init:
      1. Run `wevex serve` to start the daemon.
      2. Run `wevex sync` in your project directory to configure LLM clients.
    """
    from .auth import generate_token
    from .config import WevexConfig, _default_config_path
    ...
    token = generate_token()
    from . import paths as _wevex_paths
    effective_db = db_path or str(_wevex_paths.default_db_path())
    ...
    if auto_upgraded:
        ...
        embed_note = (
            ...
            "[bold]wevex config set embedding_provider fastembed[/bold]."
        )

    # Seed the default scope in the DB (deferred until serve, but record it now)
    from rich.panel import Panel
    console.print(Panel.fit(
        f"[bold green]✓ Wevex initialised[/bold green]\n\n"
        ...
        f"{embed_note}\n\n"
        f"Next steps:\n"
        f"  1. [bold]wevex serve[/bold]   — start the daemon\n"
        f"  2. [bold]wevex sync[/bold]    — configure LLM clients\n"
        f"  3. [bold]wevex hooks install[/bold]  — turn on autonomous mode",
        title="Wevex Init",
    ))
...
def serve(
    ...
    """Start the Wevex daemon (FastAPI + MCP on 127.0.0.1:8765).
    ...
    if not cfg.bearer_token:
        err_console.print(
            ...
            "Run [bold]wevex init[/bold] first."
        )
        sys.exit(1)

    # Iter 35: refuse to start if another daemon already holds the
    # single-instance lock. Catches the case where a second `wevex serve
    ...
    _lock_handle = _single_instance.acquire_or_exit(
        _wevex_paths.daemon_lock_file(), stderr=sys.stderr,
    )

    # Iter 28 Windows port: when `wevex serve` is launched by a Windows
    ...
    # ``wevex_home() / "logs"`` whenever we detect we're running headless
    ...
    if not _sys.stdout.isatty():
        from . import paths as _wevex_paths
        log_dir = _wevex_paths.daemon_log_dir()
    ...
    console.print(
        f"[bold green]▶ Starting Wevex daemon[/bold green] "
...
@click.option("--remove", "do_remove", is_flag=True, default=False,
              help="Disconnect CLIENT_ID instead of connecting it. With "
                   ...
                   "Replaces `wevex disconnect` per ADR-002.")
def connect(
    ...
    """Pick which installed LLM tools should share context via Wevex.

    \b
    Forms:
      wevex connect              interactive checklist of detected tools
      wevex connect cursor       connect a single client by id
      wevex connect --all        connect every detected client (CI-friendly)
      wevex connect cursor --remove  disconnect a single client
      wevex connect --all --remove   disconnect every connected client
    """
    ...
    if client_id:
        ...
        if not rows:
            ...
            ui.hint("Then re-run [bold]wevex connect[/bold].")
            return
...
def remember(
    ...
    \b
    Examples:
        wevex remember "use Redis for session caching" --type decision
        wevex remember "rate limit is 1000 req/min" --type fact --territory backend/api
        wevex remember "prefer async/await over callbacks" --type preference
    """
    cfg = _get_config()
    scope_handle = _resolve_scope(scope)
    ...
    if resp.status_code == 201:
        ...
        ui.hint(f"Create it: [bold]wevex scope create {scope_handle}[/bold]")
        sys.exit(1)
    else:
...
def recall(
    ...
    \b
    Examples:
        wevex recall "caching strategy"
        wevex recall "auth middleware" --type decision --type observation
        wevex recall "rate limits" --territory backend/api --limit 5
    """
    cfg = _get_config()
    scope_handle = _resolve_scope(scope)
...
def status(output_json: bool) -> None:
    ...
    Per ADR-002, this is the single \"is Wevex on and wired up?\" surface —
    ...
    with _client() as client:
        try:
            ...
            if output_json:
                ...
                ui.header("Wevex is offline", state="off")
                ui.hint("Run [bold]wevex up[/bold] to start the daemon.")
            sys.exit(1)

    cfg = _get_config()

    # ADR-002: fold the `clients` table into status so the user sees in
    # one screen which LLM tools are actually wired through Wevex.
    ...
    ui.header("Wevex is running", state="ok")
    ui.fields([
...
@click.option("--since", "since_arg", default=None,
              help="Show fragments created after a timestamp instead of the "
                   ...
                   "(`1h`, `2d`, `2026-05-12`). Replaces `wevex since`.")
@click.option("--json", "output_json", is_flag=True, default=False,
...
def doctor(show_perf: bool, do_clean: bool, do_reingest: bool,
    ...
    and `gc` commands. The same surface answers "is Wevex healthy?" and
    "what's it actually holding?"

    Optional flags:
      --clean      run the interactive cleanup (replaces `wevex gc`)
      --reingest   re-ingest a codebase path (replaces `wevex ingest`)

    Checks:
      - Config file exists and token is set
      - Daemon responds
      - Claude Code has wevex registered
    ...
    ui.section("Wevex doctor")
    ui.blank()

    # Config
    check("Config file", cfg_path.exists(),
          f"Not found at {cfg_path}", "Run `wevex init`")

    if cfg_path.exists():
        cfg = get_config()
        check("Bearer token", bool(cfg.bearer_token),
              "Token is empty", "Run `wevex init --force`")
        check("DB path", bool(cfg.db_path),
              "db_path not set", "Run `wevex init`")
        ...
        try:
            ...
            check("Daemon running", resp.status_code == 200,
                  f"HTTP {resp.status_code}", "Run `wevex serve`")
        except Exception as e:
            check("Daemon running", False, str(e), "Run `wevex serve`")
        ...
        if shutil.which("claude"):
            try:
                ...
                claude_ok = "wevex" in result.stdout
            except Exception:
                pass
        check("Claude Code — wevex registered", claude_ok,
              "wevex not in `claude mcp list`", "Run `wevex sync`")

        # AGENTS.md
        agents_md = Path.cwd() / "AGENTS.md"
        check("AGENTS.md in cwd", agents_md.exists(),
              f"Not found at {agents_md}", "Run `wevex sync`")
        ...
        if ep == "hash":
            check("Embedding provider", False,
                  ...
                  "Run `wevex config set embedding_provider fastembed` for "
                  "local semantic embeddings (no API key needed)")
        elif ep == "bm25":
            ui.step("Embedding provider", state="ok",
                    detail="bm25 (keyword-only — `wevex config set embedding_provider fastembed` "
            ...
            check("Embedding provider (openai, cloud)", has_key,
                  ...
                  "`wevex config set embedding_provider fastembed` for the local default")
        else:
            check("Embedding provider", False,
        ...
        # `wevex ingest . --reset` re-embeds with the new provider.
        try:
            ...
            if stored_dim and prov_dim and stored_dim != prov_dim:
                check("Embedding dimension match", False,
                      ...
                      "Re-embed with `wevex ingest . --reset`")
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                check("Chunks index sanity", ok, detail,
                      "Run `wevex chunks delete-root <name>` to clean up "
                      "and re-ingest with `wevex up` from inside the repo")
            finally:
                st.close()
        except Exception:
            pass
    ...
    # --- Optional: perf measurements (`wevex doctor --perf`) ---
...
def hook() -> None:
    ...
    Claude Code via .claude/settings.json. Run `wevex hooks install` to wire
...
def config_set(key: str, value: str) -> None:
    """Set a config key. Example: wevex config set embedding_provider fastembed"""
    from .config import WevexConfig, _default_config_path, load_config
...
def tail(
    ...
    """Tail the Wevex event log (recall, remember, supersede, leases, …).

    By default prints the last N events and then follows new ones live.
    Stop with Ctrl-C. The event log lives at ``~/.config/wevex/events.jsonl``
    (override via ``WEVEX_EVENTS_PATH``).
    """
    ...
    if not path.exists():
        err_console.print(
            ...
            "or run [bold]wevex up[/bold] in a project to start the watcher."
        )
        if not follow:
            return
...
def tui(scope: Optional[str]) -> None:
    """Launch the Wevex control-panel TUI.
