...
This endpoint exists so the Wevex extension can self-pair with the
local daemon *without* the user copy-pasting a bearer token from
``~/.config/wevex/config.json``. The pairing flow is:

  1. User installs the Wevex extension from the Chrome Web Store.
...
read ``~/.config/wevex/config.json`` directly, so this endpoint adds no
...
logger = logging.getLogger("wevex.pair")
...
async def pair_browser(request: Request) -> JSONResponse:
    """Return the local daemon's bearer token + MCP URL to a Wevex
    ...
    if not cfg.bearer_token:
        return JSONResponse(
            {"error": "Wevex not initialised. Run `wevex init` first."},
            status_code=503,
        )
