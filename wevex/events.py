...
R-02: lets the user run ``wevex tail`` to watch in real time what the daemon is
...
- Single JSONL file at ``$WEVEX_EVENTS_PATH`` (env override) or
  ``~/.config/wevex/events.jsonl`` (default). Sits next to ``wevex.db``.
...
logger = logging.getLogger("wevex.events")
...
def default_path() -> Path:
    env = os.environ.get("WEVEX_EVENTS_PATH")
    if env:
        return Path(env)
    from . import paths as _wevex_paths
    return _wevex_paths.events_jsonl_path()
...
def reset_event_logger() -> None:
    ...
    picks up a fresh ``WEVEX_EVENTS_PATH``."""
    global _LOGGER
    _LOGGER = None
