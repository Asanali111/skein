"""SQLite-backed storage layer for Wevex.
...
logger = logging.getLogger("wevex.storage")
...
class Storage:
    """Thread-safe SQLite storage for Wevex."""
    ...
    def _tune_pragmas(self) -> None:
        ...
        # via WEVEX_SQLITE_CACHE_KB for power users with huge stores.
        cache_kib = int(os.environ.get("WEVEX_SQLITE_CACHE_KB", "16384"))
        ...
        # at ~40 MiB today. Tunable via WEVEX_SQLITE_MMAP_MB.
        mmap_mb = int(os.environ.get("WEVEX_SQLITE_MMAP_MB", "64"))
    ...
    def list_mcp_clients(self) -> list[dict[str, Any]]:
        """All registered clients, newest first. Used by `wevex clients`."""
    ...
    # write here. Users approve via ``wevex inbox approve <id>``.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Migrations — kept inline because Wevex deliberately ships a single
    ...
    def peek_embedding_dimension(self) -> Optional[int]:
        ...
        stored, recall results will be nonsense until `wevex ingest . --reset`.
