"""Wevex configuration — loaded from ~/.config/wevex/config.json at startup.

The config file is created by ``wevex init`` and looked up in order:
  1. $WEVEX_CONFIG env var (path to the JSON file)
  2. ~/.config/wevex/config.json

Any key can be overridden by an environment variable with the prefix WEVEX_
(uppercase, underscores).  E.g. WEVEX_PORT=9000 overrides config["port"].
"""
...
DEFAULTS: dict[str, Any] = {
    "port": 8765,
    "host": "127.0.0.1",
    # Resolved via wevex.paths so Windows lands in %APPDATA%\wevex\ while
    # macOS / Linux stays at ~/.config/wevex/ unchanged. See wevex/paths.py.
    "db_path": str(_wevex_paths.default_db_path()),
    ...
    # mapped to ``"fastembed"`` by load_config()/WevexConfig.__init__. See
    # wevex/embeddings.py — the Gemini *embedding* API was removed in
    # iter 27 because its rate limits wedged the daemon's event loop. The
    # **Gemini CLI** LLM client (wevex/clients.py::GeminiCLIClient) is
    ...
    # rendered output's hash changes (replaces `wevex sync`). Auto-approve
    ...
    # `wevex inbox approve / reject / auto-approve`).
    ...
    # Iter 28 boot-perf: code + docs scanner moved off the `wevex up` hot
    ...
    # overridable via WEVEX_FASTEMBED_IDLE_SECONDS — this knob is just
    # the polling cadence.
...
class WevexConfig:
    """Holds runtime configuration for the Wevex daemon."""
...
def _default_config_path() -> Path:
    env = os.environ.get("WEVEX_CONFIG")
    if env:
        return Path(env)
    return _wevex_paths.default_config_path()
...
def _load_dotenv_file(path: Path) -> None:
    ...
    ``OPENAI_API_KEY`` in ``~/.config/wevex/.env`` so the daemon picks them
...
def load_config(path: Optional[Path] = None) -> WevexConfig:
    """Load config from disk, then overlay WEVEX_* env vars.
    ...
    """
    p = path or _default_config_path()
    # Iter 15: source ``~/.config/wevex/.env`` (alongside the JSON config) so
    ...
    # Run before env-var overlay so a session-scoped WEVEX_EMBEDDING_PROVIDER
    ...
    if legacy_on_disk == "gemini":
        import logging
        logging.getLogger("wevex.config").warning(
            ...
            "until you re-ingest — run `wevex up` from each project root."
        )
...
_config: Optional[WevexConfig] = None


def get_config() -> WevexConfig:
    global _config
    if _config is None:
        _config = load_config()
    return _config
...
def reset_config(cfg: Optional[WevexConfig] = None) -> None:
