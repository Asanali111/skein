...
- **Incremental ingest.** Each chunk is keyed by (scope, root, path,
  ...
  ``wevex ingest`` against the same root reuses chunks whose hash hasn't
...
logger = logging.getLogger("wevex.ingest")
...
# Path roots Wevex refuses to ingest from — a project directory should never
# match any of these. Trying to run `wevex up` here exits with an error.
_SYSTEM_ROOTS = {
