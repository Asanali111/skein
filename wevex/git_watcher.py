...
Iter 15: this is the *primary* decision-capture path for Wevex.
...
- ``MultiProjectGitWatcher`` — walks every Wevex-up'd project (those with
  ``.wevex/scope`` pin), runs a per-project watcher for each.
...
logger = logging.getLogger("wevex.git_watcher")
...
def read_commits_since(repo: Path, since_sha: Optional[str] = None,
    ...
    on ``feat/*`` and ``experiment/*`` branches reach Wevex without first
...
def discover_scoped_projects(client_root: Path = None) -> list[Path]:
    """Find every directory that has a Wevex scope pin AND a .git folder.
    ...
    for entry in sorted(root.iterdir()):
        ...
        if not path:
            continue
        if not (path / ".wevex" / "scope").is_file():
            continue
...
class MultiProjectGitWatcher:
    """Polls every Wevex-up'd, git-tracked project for new commits."""
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="wevex-git-watcher", daemon=True,
        )
        self._thread.start()
    ...
    def poll_once(self) -> dict[str, int]:
        ...
        try:
            ...
            for project_path in discover_scoped_projects(self.client_root):
                scope_handle = (project_path / ".wevex" / "scope").read_text().strip()
