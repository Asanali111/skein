...
----------------------------------------------------------------------------
There is no Gemini *embedding* provider in Wevex. The previous
...
``wevex/clients.py::GeminiCLIClient`` (id ``"gemini_cli"``). That client is
a sync target — Wevex writes an MCP config snippet so the ``gemini`` CLI
...
logger = logging.getLogger("wevex.embeddings")
...
class EmbeddingProvider:
    ...
    # to FTS5/BM25. ``wevex doctor`` and the README rely on this marker.
    is_real: bool = False
...
class HashEmbeddingProvider(EmbeddingProvider):
    ...
    network access. **Not for production use.** ``wevex doctor`` warns when
...
class FastembedProvider(EmbeddingProvider):
    ...
    default so `pip install wevex && wevex up` produces semantic search
    ...
    def _ensure_model(self):
        if self._model is None:
            ...
            cache_dir = Path(
                os.environ.get("WEVEX_FASTEMBED_CACHE")
    ...
    def idle_check_and_unload(self) -> bool:
        ...
        try:
            window = float(
                os.environ.get("WEVEX_FASTEMBED_IDLE_SECONDS",
    ...
    if name == "gemini":
        if not _gemini_alias_warned:
            logger.warning(
                ...
                "'fastembed' (local, 384-dim). Run `wevex up` to migrate "
