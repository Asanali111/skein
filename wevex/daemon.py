...
The local Wevex daemon should be a *background service* that:
  • starts on first activation (`wevex up`)
  • survives terminal close
  • restarts automatically on reboot

Three backends, picked in this order:
  1. **launchd** (macOS): writes ~/Library/LaunchAgents/com.wevex.daemon.plist
     and ``launchctl bootstrap``s the user agent.
  2. **systemd-user** (Linux): writes ~/.config/systemd/user/wevex.service
     and ``systemctl --user enable --now``s it.
  3. **nohup** (Windows + Linux-without-systemd + macOS with ``--no-persist``):
     spawns a detached process via :mod:`wevex._proc` and stores the PID at
     ...
     **not** reboot — there's no native auto-start. On Windows, restart-on-
     login can be added later via a Scheduled Task pointing at ``wevex up``.
...
logger = logging.getLogger("wevex.daemon")
...
# their native locations regardless of where Wevex's data dir is.
LAUNCHD_LABEL = "com.wevex.daemon"
LAUNCHD_PLIST = Path.home() / "Library" / "LaunchAgents" / f"{LAUNCHD_LABEL}.plist"

SYSTEMD_UNIT_NAME = "wevex.service"
...
# grouping under Task Scheduler ▸ Task Scheduler Library ▸ Wevex. Task runs
# at user logon with restart-on-failure baked into the XML so the daemon
# stays alive across reboots without admin elevation.
SCHTASKS_TASK_NAME = r"Wevex\Daemon"

# Nohup PID + log dir live under WEVEX_HOME — these *do* move on Windows
# (to %APPDATA%\wevex\). See wevex/paths.py.
NOHUP_PID_FILE = _wevex_paths.daemon_pid_file()
DAEMON_LOG_DIR = _wevex_paths.daemon_log_dir()
...
def ensure_running(*, persist: bool = True, base_url: str = "",
                   wevex_bin: Optional[str] = None) -> DaemonStatus:
    ...
    wevex_bin = wevex_bin or _resolve_wevex_bin()
    ...
    if backend == "launchd":
        _install_launchd(wevex_bin)
    elif backend == "systemd":
        _install_systemd(wevex_bin)
    elif backend == "schtasks":
        _install_schtasks(wevex_bin)
    else:
        _start_nohup(wevex_bin)
...
def restart(*, persist: bool = True, base_url: str = "",
    ...
    stop()
    time.sleep(0.3)
    return ensure_running(persist=persist, base_url=base_url, wevex_bin=wevex_bin)
...
_BACKEND_CACHE_FILE = _wevex_paths.backend_cache_file()
...
def _quick_backend_label() -> str:
    ...
    on a fresh install. If a real Wevex scheduled task was registered,
...
def relocate_venv_to_wevex_home(*, package_source: Optional[Path] = None) -> Path:
    """Build a TCC-safe venv at ``~/.wevex/venv`` and re-link the global ``wevex``.

    Returns the path to the new ``wevex`` binary inside the relocated venv.
    Used by ``wevex up`` when the current install lives in a protected dir.
    """
    target_home = Path.home() / ".wevex"
    target_home.mkdir(exist_ok=True)
    target_venv = target_home / "venv"

    # Locate the package source (where pyproject.toml lives)
    if package_source is None:
        import wevex as _wevex
        package_source = Path(_wevex.__file__).parent.parent
    if not (package_source / "pyproject.toml").is_file():
        raise RuntimeError(
            f"Cannot relocate: source dir {package_source} has no pyproject.toml"
        )

    # The relocated venv must NOT be inside a TCC-protected dir; ~/.wevex never is.
    # If the source itself is in Documents we copy it to ~/.wevex/source first so
    ...
    # Update common PATH symlinks so `wevex` everywhere points at the new venv
    for link in ("/usr/local/bin/wevex", str(Path.home() / ".local" / "bin" / "wevex")):
        path = Path(link)
        try:
...
def _resolve_wevex_bin() -> str:
    """Return the path to the `wevex` executable to put in service files.

    Prefer the running interpreter's venv (so launchd / systemd / schtasks
    use the same venv that ran `wevex up`). Fall back to PATH lookup.

    Windows venvs lay the binary under ``Scripts\\wevex.exe``; POSIX
    venvs use ``bin/wevex``. Check both layouts so this works regardless of
    whether `wevex up` was invoked from a Windows venv or a POSIX one.
    """
    if hasattr(sys, "prefix") and Path(sys.prefix).is_dir():
        prefix = Path(sys.prefix)
        candidates = [
            prefix / "Scripts" / "wevex.exe",  # Windows venv
            prefix / "Scripts" / "wevex",      # Windows console-script (rare)
            prefix / "bin" / "wevex",          # POSIX venv
        ]
    ...
    if found:
        return found
    raise RuntimeError(
        "Could not find the `wevex` executable. "
        "Re-run from the venv where Wevex is installed, "
        "or pass --wevex-bin to wevex up."
    )
...
def _check_health(base_url: str = "http://127.0.0.1:8765") -> bool:
    ...
    critical path of every ``wevex up``. For a single one-shot GET to a
...
def _install_launchd(wevex_bin: str) -> None:
    ...
    # bootstrap-started instance keep running collapses cold `wevex up`
    # from ~20–25 s to ~2 s.
...
Description=Wevex local MCP context bus
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart={wevex_bin} serve
Restart=always
RestartSec=3
...
def _install_systemd(wevex_bin: str) -> None:
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>Local MCP context bus for coding LLMs (managed by `wevex up`).</Description>
...
def _install_schtasks(wevex_bin: str) -> None:
    DAEMON_LOG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    xml = _SCHTASKS_XML_TEMPLATE.format(
        ...
        wevex_bin=_xml_escape(wevex_bin),
    ...
    # /F forces overwrite if the task already exists — keeps `wevex up`
...
def _uninstall_schtasks(silent: bool = False) -> None:
    ...
    try:
        (_wevex_paths.wevex_home() / "schtasks.xml").unlink()
    except OSError:
        pass
...
def _start_nohup(wevex_bin: str) -> None:
    DAEMON_LOG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    # spawn is in wevex/_proc.py — uses start_new_session on POSIX,
