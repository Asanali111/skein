...
things:
  ...
  connect()     — write the config files needed to point the client at Wevex
  disconnect()  — surgically remove Wevex's blocks from those config files
...
def _delete_if_empty_or_orphan(path: Path, key_chain: list[str]) -> bool:
    """If a JSON file's mcpServers (or other root) is empty after removing wevex,
...
class BaseClient:
    ...
    def disconnect(self, recorded_paths: Optional[list[str]] = None) -> list[str]:
        """Remove wevex from this client's config; return list of paths
...
class ClaudeCodeClient(BaseClient):
    ...
    def connect(self, mcp_url, bearer_token, scope_handle, repo) -> list[str]:
        ...
        return [f"claude:mcp:wevex@{mcp_url}"]
...
class CursorClient(BaseClient):
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        return _remove_wevex_from_json(
            recorded_paths or [],
            ["mcpServers"],
...
def _opencode_config_dir() -> Path:
    ...
    would still work but Wevex wouldn't actually be wired up.
    """
...
class CodexClient(BaseClient):
    ...
    def connect(self, mcp_url, bearer_token, scope_handle, repo) -> list[str]:
        ...
        # Iter 18.6+: strip any stale wevex block before appending a fresh one,
        # so a token rotation propagates here instead of being silently ignored.
        # The previous "if wevex in existing: return" guard meant the codex
        # config kept the dead leaked token after iter-16's rotation — caught
        # during iter 18 by a security sweep.
        cleaned = _strip_codex_wevex_block(existing)
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        ...
        for path in candidates:
            if not path.exists():
                continue
            text = path.read_text()
            if "wevex" not in text:
                continue
            # Hand-rolled TOML edit — strip the [[mcpServers]] block whose
            # name = "wevex" plus its [mcpServers.headers] sub-table.
            cleaned = _strip_codex_wevex_block(text)
            if cleaned != text:
...
def _strip_codex_wevex_block(text: str) -> str:
    """Remove the wevex-related TOML blocks from a codex config.
    ...
    We strip from ``[[mcpServers]]`` (with name = "wevex") through the next
    ...
    while i < n:
        ...
        if stripped == "[[mcpServers]]":
            # Look ahead to see if this block is the wevex one.
            j = i + 1
            block: list[str] = [line]
            is_wevex = False
            while j < n:
...
def _remove_wevex_from_json(
    recorded_paths: list[str],
    key_chain: list[str],
    default_paths: list[Path],
) -> list[str]:
    """Walk into ``data[key_chain[0]][key_chain[1]]…`` and pop ``"wevex"``.
    ...
    for path in candidates:
        ...
        for key in key_chain:
            if not isinstance(node, dict):
                break
            node = node.get(key, {})
        if isinstance(node, dict) and "wevex" in node:
            del node["wevex"]
            _write_json(path, data)
            modified.append(str(path))
    return modified
...
class GooseClient(BaseClient):
    ...
    def connect(self, mcp_url, bearer_token, scope_handle, repo) -> list[str]:
        ...
        data["extensions"]["wevex"] = {
            "enabled": True,
            "type": "streamable_http",
            "name": "wevex",
            "description": "Wevex MCP context bus",
            "uri": mcp_url,
            "headers": {"Authorization": f"Bearer {bearer_token}"},
            "timeout": 300,
        }
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        ...
        for path in candidates:
            ...
            if isinstance(exts, dict) and "wevex" in exts:
                del exts["wevex"]
                _write_yaml(path, data)
                modified.append(str(path))
        return modified
...
def _strip_gptme_wevex_block(text: str) -> str:
    """Remove the wevex ``[[mcp.servers]]`` block from a gptme config.
    ...
    ``name = "wevex"`` through the next blank line or top-level table header.
    """
    lines = text.splitlines(keepends=True)
    out: list[str] = []
    i = 0
    ...
    while i < n:
        ...
        if stripped == "[[mcp.servers]]":
            # Peek ahead to see if this block belongs to wevex
            j = i + 1
            block: list[str] = [line]
            is_wevex = False
            while j < n:
...
class GptmeClient(BaseClient):
    ...
    def connect(self, mcp_url, bearer_token, scope_handle, repo) -> list[str]:
        ...
        # Strip any stale wevex block before appending a fresh one so that a
        # token rotation propagates instead of being silently ignored
        # (same lesson as the iter-18.6 Codex fix).
        cleaned = _strip_gptme_wevex_block(existing)
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        ...
        for path in candidates:
            ...
            if "wevex" not in text:
                continue
            cleaned = _strip_gptme_wevex_block(text)
            if cleaned != text:
                path.write_text(cleaned)
                modified.append(str(path))
        return modified
...
class HermesClient(BaseClient):
    ...
    def connect(self, mcp_url, bearer_token, scope_handle, repo) -> list[str]:
        ...
        # Write token to .env
        _write_hermes_env_key(env_path, "MCP_WEVEX_API_KEY", bearer_token)
        ...
        if config_path.exists():
            try:
                ...
                config = {}
        config.setdefault("mcp_servers", {})["wevex"] = {
            "url": mcp_url,
            "headers": {"Authorization": "Bearer ${MCP_WEVEX_API_KEY}"},
        }
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        ...
        if config_path.exists():
            ...
            if isinstance(servers, dict) and "wevex" in servers:
                del servers["wevex"]
                if not servers:
        ...
        if env_path.exists():
            _write_hermes_env_key(env_path, "MCP_WEVEX_API_KEY", "")
...
class ContinueClient(BaseClient):
    ...
    def disconnect(self, recorded_paths=None) -> list[str]:
        # The block file is entirely Wevex-owned — safe to delete outright.
        candidates: list[Path] = []
        seen: set[str] = set()
