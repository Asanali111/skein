...
their chat, but Wevex has no equivalent reset — so the value of a fragment
...
* Provenance prior — the *origin* of the fragment is the strongest cheap
  signal. A human typing ``wevex remember`` is near-certainly worth
    ...
    *,
    ...
    if em == "explicit":
        ...
        if metadata.get("has_alternatives") or metadata.get("structured_decision"):
            return 0.90
        # CLI-typed `wevex remember` arrives without a tool label — the
        # MCP path stamps "claude-code" / "cursor" / etc.
        if tool in ("", "cli", "wevex-cli", "human"):
            return 1.00
        return 0.70
