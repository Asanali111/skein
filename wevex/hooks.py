...
Each handler is a thin function that:
  ...
  2. Talks to the local Wevex storage directly (no HTTP — daemon may be off)
...
they call the same `wevex recall` / `wevex remember` CLI rather than these
...
logger = logging.getLogger("wevex.hooks")
...
def session_start(stdin_text: str = "") -> int:
    ...
    try:
        ...
        text = _render_grouped(scope_handle, top, header="Wevex context")
        out = {
            "hookSpecificOutput": {
...
def user_prompt_submit(stdin_text: str = "") -> int:
    ...
    text = _render_grouped(
        scope_handle, frags,
        header=f"Wevex recall — `{prompt[:60]}…`",
    )
    out = {
        "hookSpecificOutput": {
