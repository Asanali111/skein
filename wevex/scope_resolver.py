...
Resolution order (first match wins):
  ...
  2. ``WEVEX_SCOPE`` env var (set by hooks_install for Claude Code hooks)
  3. ``.wevex/scope`` file in cwd or any parent (written by ``wevex hooks install``)
  4. ``cfg.default_scope``

This is shared between the hook handlers (wevex/hooks.py) and the human-facing
CLI commands (wevex/cli.py) so a project-level pin applies uniformly to every
``wevex …`` invocation, not just the ones called from inside Claude Code.
"""
...
def auto_detect_scope(start: Optional[Path] = None) -> str:
    ...
    Priority:
      1. Existing ``.wevex/scope`` file in cwd or any parent.
...
def find_scope_pin(start: Optional[Path] = None) -> Optional[str]:
    """Walk up from ``start`` (or cwd) looking for a ``.wevex/scope`` file.

    Returns the pinned scope handle, or None if no pin file is found.
    """
    cwd = (start or Path.cwd()).resolve()
    for parent in [cwd, *cwd.parents]:
        f = parent / ".wevex" / "scope"
        if f.exists():
            try:
...
def resolve_scope(
    ...
    *,
    ...
    Returns ``(scope_handle, source)`` where ``source`` is one of:
      ...
      - ``"env"``  — WEVEX_SCOPE env var
      - ``"pin"``  — .wevex/scope file
    ...
    env_scope = os.environ.get("WEVEX_SCOPE")
    if env_scope:
    ...
    raise RuntimeError(
        "Could not resolve scope. Pass --scope, set WEVEX_SCOPE, "
        "create a .wevex/scope file (run `wevex hooks install`), "
