"""OS-level single-instance enforcement for ``wevex serve``.

Acquires an advisory file lock on ``wevex_home() / "daemon.lock"`` before
the daemon starts. The kernel auto-releases the lock when the holding
process dies, so there's no stale-PID cleanup problem.

Without this, a second ``wevex serve --port N`` (N != configured port)
...
class LockResult:
    ...
    file was created by an older wevex that didn't write a PID).
    """
    acquired: bool
...
def acquire_or_exit(lock_path: Path, *, stderr=None) -> object:
    ...
    print(
        f"wevex: another daemon is already running ({pid_str}). "
        f"Use `wevex down` to stop it first, then `wevex up` again. "
