...
logger = logging.getLogger("wevex.watcher")
...
class _WatchdogWatcher(_BaseWatcher):
    ...
    def start(self) -> None:
        ...
        threading.Thread(
            target=self._drain_loop, name="wevex-watcher-drain", daemon=True,
        ).start()
...
class _PollingWatcher(_BaseWatcher):
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._loop, name="wevex-watcher-poll", daemon=True,
        )
        self._thread.start()
