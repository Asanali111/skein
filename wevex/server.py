"""FastAPI application factory for the Wevex daemon.
...
from .config import WevexConfig, get_config
...
logger = logging.getLogger("wevex.server")
...
async def lifespan(app: FastAPI):
    ...
    cfg: WevexConfig = app.state.cfg  # type: ignore[attr-defined]
    ...
    # "Wevex is slow." Failures here must NOT crash the daemon; we log
    ...
    # until the user re-embeds with `wevex ingest . --reset`. We don't auto-
    # invalidate (irreversible); just log the situation clearly so it shows
    # up in `wevex doctor` and the daemon stderr log.
    try:
    ...
    # are spawned as session-scoped subprocesses by `wevex up` instead, so
    ...
    # from `wevex up`, costing 2–4 s every warm boot. Daemon-side sweep
    ...
    # Every new commit in a Wevex-up'd repo becomes a `decision` fragment
    ...
    #   - WEVEX_TRANSCRIPT_WATCHER=loose → run the full pattern set, including
    #     the lower-confidence "let's use X" / "I prefer X" patterns that
    #     queue to inbox for review.
    #   - WEVEX_TRANSCRIPT_WATCHER=off → disable entirely.
    transcript_watcher = None
    _twc_mode = os.environ.get("WEVEX_TRANSCRIPT_WATCHER", "smart").lower()
...
def create_app(cfg: Optional[WevexConfig] = None) -> FastAPI:
    if cfg is None:
        cfg = get_config()
...
async def _agents_md_sync_loop(db_path: str, daemon_url: str, interval: int) -> None:
    """ADR-002 / iter 26: replace `wevex sync` with a daemon-side regen.
...
async def _inbox_auto_approve_loop(db_path: str, cfg, provider, interval: int) -> None:
    """ADR-002 / iter 26: replace `wevex inbox auto-approve` with a daemon
...
async def _passive_scan_loop(db_path: str, cfg, provider, interval: int) -> None:
    ...
    daemon side so ``wevex up`` doesn't pay the cost on every invocation.
    ...
    # an empty `recall` for 5+ s after `wevex up` returned. Cold-start
