...
Persistent state at ``~/.config/wevex/connections.json``::

    {
      "cursor": {
        ...
        "config_paths": [
          ...
          "/Users/ameliomar/proj/.cursor/rules/wevex.mdc"
        ]
      },
      ...
    }

The registry is the source of truth for ``wevex sync`` and ``wevex
disconnect``. ``wevex up`` syncs only the clients listed here; ``wevex
disconnect cursor`` reads the recorded paths and surgically removes the
wevex block from each.
"""
...
# Moves to %APPDATA%\wevex\connections.json on Windows; unchanged on
# macOS / Linux. See wevex/paths.py.
CONNECTIONS_PATH = _wevex_paths.connections_path()
