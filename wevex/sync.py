"""Client config sync — orchestrator over ``wevex.clients``.

``wevex sync`` writes per-client MCP configs and, optionally, the universal
AGENTS.md / CLAUDE.md fallback. The actual per-client logic lives in
``wevex.clients`` so the connect/disconnect surfaces stay symmetric.

By default, ``sync_all`` writes only for clients that the user has marked
connected (via ``wevex connect`` → ``connections.json``). Pass
``client_ids=[...]`` to override (used internally by ``wevex connect``
...
logger = logging.getLogger("wevex.sync")
...
def sync_all(
    ...
    ----------
    daemon_url:
        Base URL of the Wevex daemon, e.g. "http://127.0.0.1:8765".
    bearer_token:
    ...
    client_ids:
        ...
        user has previously connected via ``wevex connect``.
    """
