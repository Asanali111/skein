    ...
    if not cfg.bearer_token:
        return JSONResponse(
            {"jsonrpc": "2.0", "id": None, "error": {
                "code": -32000,
                "message": "Wevex not initialised. Run `wevex init` first.",
            }},
            status_code=503,
        )
...
def _handle_initialize(params: dict, storage: Any) -> dict:
    return {
        ...
        "serverInfo": {"name": "wevex", "version": "0.1.0"},
        "capabilities": {
...
def _build_initialize_instructions(storage: Any) -> str:
    ...
    if fragment_count == 0:
        ...
        if activity:
            ...
            lines.append(
                "No writes in the last 24 h — Wevex is quiet. Connecting "
                "another LLM (`wevex connect`) compounds the value: every "
...
_TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "cosine similarity; if the top result is `quality=none`, Wevex has "
    ...
    },
    {
        ...
        "description": (
            ...
            "follow-up to that recall — lets Wevex learn which recalls "
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "from_recall": {"type": "string", "description": "Optional recall_id from a prior recall() this write follows from. Lets Wevex link recalls to outcomes for value tuning."},
            },
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "scope": {"type": "string", "description": "Scope handle. Omit to auto-detect."},
                "from_recall": {"type": "string", "description": "Optional recall_id from a prior recall() this note follows from. Lets Wevex link recalls to outcomes for value tuning."},
            },
            "required": ["content"],
        },
    },
    ...
    },
    {
        ...
        "description": (
            ...
            "new fragment AND surfaces the supersede chain — uniquely Wevex's "
    ...
    },
    {
        ...
        "description": (
            ...
            "tokens per snippet. Code must be ingested via `wevex ingest` first."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "we decide X?\" — Wevex has the full origin story even if the "
...
def build_briefing(storage: Any, scope_handle: str) -> dict[str, Any]:
    ...
    if active_inbox_count > 0:
        next_action = (
            f"Review {active_inbox_count} pending fragments via wevex inbox"
        )
...
async def _call_tool(
    ...
    # precedence as the CLI (.wevex/scope > WEVEX_SCOPE > config default).
    ...
    if name == "recall":
        ...
        if is_low_signal:
            ...
            if len(query) >= 10 and " " in query.strip():
                ...
                # thinking Wevex is empty when actually the store has
                ...
                try:
                    ...
                    total_in_scope = None
                total_note = (
                    f" (Wevex has {total_in_scope} fragments in this scope, "
                ...
                if not response.results:
                    preface = (
                        f"No fragment in Wevex matched {query!r}.{total_note} "
                    )
                else:
                    preface = (
    ...
    if name == "note":
        ...
        return _tool_text(
            ...
            f"Wevex classified it automatically — call recall_one if you need to verify."
        )
    ...
    if name == "search_code":
        ...
        if not response.results:
            return _tool_text(
                ...
                f"Has the codebase been ingested? Run `wevex ingest <path>` first."
            )
...
_PROMPTS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "agent always queries Wevex before answering project-specific "
...
_RECALL_FIRST_TEXT = """\
You have access to Wevex, a shared context bus across every coding LLM \
...
Wevex is the single source of truth for cross-session, cross-agent project \
