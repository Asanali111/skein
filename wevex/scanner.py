...
``wevex up`` (and the watcher) can promote into real fragments.
...
- **Conservative.** Every fact has a confidence score; only the very
  ...
  auto-promote. Ambiguous ones go through ``wevex inbox``.
- **Idempotent.** Same project, same scan output. Re-running on every
  ``wevex up`` is safe because the dedup index in ``extraction_candidates``
  catches duplicates.
...
logger = logging.getLogger("wevex.scanner")
...
# ---------------------------------------------------------------------------
# Promotion helpers — called by `wevex up` / the daemon
# ---------------------------------------------------------------------------
