...
Renders a canonical AGENTS.md from:
  - A header with Wevex daemon URL and how to recall context
...
``wevex up`` for first-render and by the daemon's auto-sync loop (server.py)
...
logger = logging.getLogger("wevex.agents_md")
...
def render_agents_md(
    ...
    *,
    ...
    lines.append("")
    lines.append(f"> **Generated by Wevex** at {now}. Do not edit this file directly;")
    lines.append("> the daemon regenerates it automatically when fragments change. Add custom content in the `<!-- @user -->` block.")
    lines.append("")
    lines.append("## How to use Wevex in this project — read BEFORE you do anything else")
    lines.append("")
    lines.append("Wevex is the fastest path to context here. Concrete numbers:")
    ...
    lines.append("")
    lines.append("Each `recall` result carries a `quality` bucket: `high` / `medium` / `low` / `none`. If the top result is `quality=none`, Wevex has nothing high-signal for that query — fall back to source. Honest no-knowledge is more useful than weak hits.")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## Wevex context bus")
    lines.append("")
    lines.append(f"This project uses [Wevex]({daemon_url}) for cross-LLM context sharing.")
    ...
    # Surface the implicit context Wevex has learned from the codebase + chat
    ...
    if fact_frags:
        ...
        lines.append("")
        lines.append(
            "Wevex extracted these facts from your codebase and recent AI "
...
def _extract_user_block(content: str) -> str:
    ...
    ``wevex sync``, growing AGENTS.md unboundedly. (Reproduced live: a
...
def _minimal_agents_md(scope_handle: str, daemon_url: str) -> str:
    return (
        f"# AGENTS.md\n\n"
        f"> Wevex daemon: {daemon_url}/mcp\n\n"
        f"Scope `{scope_handle}` not found. Run `wevex init` to set up.\n"
    )
...
def sync_agents_md_for_project(
    *,
    ...
    Used by both ``wevex up`` and the daemon auto-sync loop. Returns True
