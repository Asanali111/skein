...
-- ---------------------------------------------------------------------------
-- Identities
-- Every entity that touches Wevex (user, agent, LLM process) gets a row here.
...
CREATE TABLE IF NOT EXISTS fragments (
  ...
  -- so `wevex archaeology` can reconstruct decisions even years later.
...
--   content_hash: sha256 of the chunk content. Lets `wevex ingest` skip
...
-- One row per LLM client connected to Wevex. `wevex up` populates this so
...
-- `wevex inbox` review before being promoted to real fragments.
-- ---------------------------------------------------------------------------
...
-- whatever was last written. Replaces the manual `wevex sync` command.
