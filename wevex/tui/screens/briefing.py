...
Renders the same payload as ``wevex briefing`` but live in a tabbed surface:
...
class BriefingPane(Container):
    ...
    async def refresh_briefing(self) -> None:
        ...
        try:
            ...
            header.update(f"[bold]Wevex — {self._scope}[/bold]")
            stats.update(
        ...
        header.update(f"[bold]Wevex — {briefing.get('scope', self._scope)}[/bold]")
        ...
        if recent:
            ...
            decisions.update(
                ...
                "use [bold]wevex note[/bold] or call the [bold]note_decision[/bold] "
