...
class HttpDaemonClient:
    ...
    Uses the bearer token + base URL from the same WevexConfig the CLI reads.
    """
    ...
    async def _candidate_action(self, candidate_id: str, action: str) -> dict[str, Any]:
        import subprocess
        cmd = ["wevex", "inbox", action, candidate_id]
...
def build_default_client() -> HttpDaemonClient:
    """Build the production daemon client from ``WevexConfig``."""
    from ..config import get_config
...
def resolve_tui_scope(cli_scope: Optional[str]) -> str:
    ...
    Honors --scope > WEVEX_SCOPE > .wevex/scope pin > config default.
