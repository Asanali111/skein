"""Registry of active Wevex projects.

Each call to ``wevex up`` registers the project (root directory + scope) so the
daemon can:
...
Stored at ``~/.config/wevex/projects.json``:
...
logger = logging.getLogger("wevex.projects")
...
# Moves to %APPDATA%\wevex\projects.json on Windows; unchanged on
# macOS / Linux. See wevex/paths.py.
REGISTRY_PATH = _wevex_paths.projects_registry_path()
