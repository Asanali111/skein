...
iter 19: scanners in ``wevex/scanner.py`` extract implicit facts from
...
``wevex up`` supersedes old fragments instead of stacking copies.
...
logger = logging.getLogger("wevex.docs_watcher")
...
def scan_docs(repo: Path) -> list[ScannedFact]:
    ...
    Discovery patterns (all relative to ``repo``):
      ...
      - ``adr/**/*.md``, ``ADR/**/*.md``, ``decisions/**/*.md``,
        ``architecture/**/*.md``
      - ``.wevex/docs/**/*.md`` (project-specific overrides)
...
_GLOB_PATTERNS: tuple[str, ...] = (
    ...
    # accumulated rules, decisions, and preferences. A brand-new Wevex
...
# dirs are blanket-skipped (see _is_ignored) with the explicit ``.wevex/docs/``
...
def _is_ignored(repo: Path, path: Path, gitignore: set[str]) -> bool:
    ...
    # Builtin hidden-dir skip, with .wevex/docs carve-out.
    parts = rel.parts
    for part in parts[:-1]:  # parents only — the filename itself can start with "."
        if part.startswith("."):
            # Allow project-local override directory ".wevex/docs/".
            if part == ".wevex" and len(parts) >= 3 and parts[1] == "docs":
                continue
            return True
...
def _process_file(repo: Path, path: Path) -> list[ScannedFact]:
    ...
    # Iter 29 day-one: skip Wevex-generated AGENTS.md files so the docs
    ...
    # original sources.
    if "Generated by Wevex" in text[:200]:
        return []
