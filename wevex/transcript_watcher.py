...
high precision when it does catch. The review queue (``wevex inbox``)
...
logger = logging.getLogger("wevex.transcript_watcher")
...
_PATTERNS: list[ExtractionPattern] = [
    ...
    # recap but routinely forgets to `note_decision` back to Wevex. Confidence
...
def extract_from_message(
    ...
    ``WEVEX_TRANSCRIPT_WATCHER=loose``.
    """
...
class ClaudeCodeTranscriptWatcher:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="wevex-transcript-watcher", daemon=True,
        )
        self._thread.start()
...
class MultiProjectTranscriptWatcher:
    ...
    looks up the matching Wevex scope (by ``.wevex/scope`` pin OR by
    ``project:<basename>`` convention OR by scope_resolver), and if a scope
    exists, runs the extractor against any unseen bytes.

    Projects with no Wevex scope are silently skipped — opt-in by user.
    """
    ...
    def start(self) -> None:
        ...
        self._stop.clear()
        self._thread = threading.Thread(
            target=self._run, name="wevex-multi-transcript-watcher", daemon=True,
        )
        self._thread.start()
    ...
    def _resolve_scope_id(self, storage, project_path: Path) -> Optional[str]:
        ...
        Order:
          1. ``.wevex/scope`` pin file in the project root
          2. ``project:<basename>`` if it already exists in the scopes table
        """
        pin = project_path / ".wevex" / "scope"
        ...
        if not scope_handle:
            ...
            # explicitly initialized with `wevex up`.
            return None
