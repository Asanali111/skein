// Wevex content script — shared core
//
...
// only its DOM selectors and calls __WevexCommon.init(siteAdapter).
//
...
// isolated world per tab, so this file just attaches __WevexCommon to
// globalThis and the per-site files reference it on script-load.

(() => {
  const LOG = (...args) => console.info("[wevex]", ...args);
  const WARN = (...args) => console.warn("[wevex]", ...args);
  ...
  function ensureBadge() {
    ...
    badge.className = "wevex-badge";
    badge.innerHTML = `
      <span class="wevex-dot"></span>
      <span class="wevex-label">Wevex</span>
      <span class="wevex-detail">…</span>
    `;
    badge.title = "Click to open the Wevex extension popup";
    badge.addEventListener("click", () => {
      ...
      badge.classList.add("wevex-flash");
      setTimeout(() => badge.classList.remove("wevex-flash"), 400);
    });
    document.documentElement.appendChild(badge);
    return badge;
  }

  function setBadge({ kind = "ok", label = "Wevex", detail = "" } = {}) {
    const b = ensureBadge();
    b.classList.remove("wevex-ok", "wevex-warn", "wevex-err");
    b.classList.add(`wevex-${kind}`);
    b.querySelector(".wevex-label").textContent = label;
    b.querySelector(".wevex-detail").textContent = detail;
  }
  ...
  // marker on line 1: `[wevex:relevance=high|medium|low|none]`. Parse it
  ...
  function parseRelevanceAndStrip(rawText) {
    ...
    const m = /^\[wevex:relevance=(high|medium|low|none)\]\s*$/.exec(firstLine.trim());
    if (!m) {
  ...
  function formatContextBlock(recallResult, originalQuery) {
    ...
    if (/^no fragment in wevex matches/i.test(body.trim())) return { block: null, relevance: "none" };
    return {
      relevance,
      block: [
        `[Wevex context — auto-injected by browser extension for query: "${originalQuery.slice(0, 80)}", relevance=${relevance}]`,
        body.trim(),
        `[/Wevex context]`,
        "",
        "",
  ...
  function makeInterceptor(siteAdapter) {
    ...
    async function handleSubmitAttempt(originalEvent, source) {
      ...
      if (!cached.activeScope) {
        ...
        flashToast("Wevex: open the toolbar icon and pick a scope");
        return;
      }
      ...
      // Already has a Wevex block (e.g. user re-submitted after we
      // injected); don't double-inject.
      if (original.startsWith("[Wevex context")) {
        LOG("already has Wevex block; passing through");
        return;
      }
      ...
      flashToast("Wevex → recalling context…");

      try {
        const r = await callRecall(original);
        if (!r || !r.ok) {
          WARN("recall failed; passing through:", r && r.error);
          flashToast(`Wevex: recall failed (${r && r.error || "unknown"})`);
        ...
        if (!block) {
          ...
          if (relevance === "low" || relevance === "none") {
            flashToast(`Wevex: no relevant context (saved tokens)`);
            setBadge({
              kind: "warn",
              detail: `${cached.activeScope || ""} · no relevant match`,
            });
          } else {
            flashToast("Wevex: no relevant context found");
          }
          retriggerSubmit(promptEl);
          return;
        }
  ...
  // ---- Save to Wevex button on assistant turns (iter 35) -------------
  //
  // Per-turn "Save to Wevex" button. Default behavior: extract the turn's
  ...
  function makeSaveButton(turnEl, getAssistantText) {
    ...
    btn.className = "wevex-save-btn";
    btn.type = "button";
    btn.textContent = "Save to Wevex";
    btn.title = "Save this assistant turn as a Wevex note (auto-classified)";
    btn.addEventListener("click", async (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      if (saved.has(turnEl) || btn.disabled) return;
      if (!cached.activeScope) {
        flashToast("Wevex: pick a scope in the toolbar popup first");
        return;
      }
      const text = getAssistantText(turnEl);
      if (!text || text.length < 8) {
        flashToast("Wevex: nothing to save (turn is empty)");
        return;
      }
      btn.disabled = true;
      ...
      try {
        ...
        if (!r || !r.ok) {
          ...
          btn.classList.add("wevex-save-err");
          WARN("note failed:", r && r.error);
          flashToast(`Wevex: save failed (${r && r.error || "unknown"})`);
          setTimeout(() => {
            btn.disabled = false;
            btn.classList.remove("wevex-save-err");
            btn.textContent = "Save to Wevex";
          }, 2500);
          return;
        }
        ...
        btn.classList.add("wevex-save-ok");
        flashToast("Wevex → noted");
      } catch (err) {
        WARN("save error:", err);
        btn.textContent = "Save failed";
        btn.classList.add("wevex-save-err");
  ...
  function decorateAssistantTurns(siteAdapter) {
    ...
    for (const turn of turns) {
      ...
      // turn becomes relative via the .wevex-host class.
      turn.classList.add("wevex-host");
  ...
  function init(siteAdapter) {
    ...
    // Iter 35: install Save-to-Wevex buttons on assistant turns (no-op
