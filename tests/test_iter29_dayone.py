"""Iter 29 day-one bundle — tests for the four changes that make Wevex
valuable from minute zero on a fresh machine:
  ...
  #1 — cold-start corpus (docs_watcher patterns + Wevex-AGENTS.md skip)
...
from wevex import docs_watcher, mcp, storage as storage_mod
...
class TestRecentWritesByTool:
    ...
    def real_storage(self, tmp_path):
        ...
        s = storage_mod.Storage(str(tmp_path / "wevex.db"))
        yield s
        s.close()
...
# ---------------------------------------------------------------------------
# #1 — docs_watcher new patterns + Wevex-AGENTS.md skip
# ---------------------------------------------------------------------------
...
class TestDocsWatcherCorpusExpansion:
    def test_globs_include_ai_tool_config_files(self) -> None:
        ...
        # fresh `wevex up` against an existing project.
    ...
    def test_wevex_generated_agents_md_is_skipped(self, tmp_path: Path) -> None:
        ...
        agents.write_text(
            "# AGENTS.md — myproj\n\n"
            "> Generated by Wevex at 2026-05-20 10:00 UTC. Do not edit "
            "this file directly; run `wevex sync` to regenerate.\n\n"
            "## Some content\n\nthat would otherwise be harvested.\n"
        )
        result = docs_watcher._process_file(tmp_path, agents)
        assert result == [], (
            "Wevex-generated AGENTS.md must be skipped to avoid the "
            "ingest-output-of-our-own-output inflation loop"
        )

    def test_user_authored_agents_md_is_harvested(self, tmp_path: Path) -> None:
        """A user-authored AGENTS.md (no Wevex header) IS harvested — that's
